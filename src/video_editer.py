...
class Speech:
    def __init__(self, start, end, content=' ', silent=None ):
        self.start = start
        self.end = end
        ...
        for row in range(self.src_listmodel.rowCount()):
            ...
            if os.path.exists(src_path + src_name + '.srt'):
                ...
                for line in range(0, len(sub_file), 4):
                    content = sub_file[line + 2].rstrip('\n').lstrip('\n')
                    print("content:", content)
                    pos = sub_file[line + 1].find('-')
                    start, end = sub_file[line + 1][:pos-1], sub_file[line+1][pos+4:].rstrip('\n')
